...
- After the user answers all the questions, ask the user if he wants to fetch his HRV data from his smartwatch (recalling the "hrv-data" skill) before asking the Doctor Agent for a diagnosis. Do not show again the "Recommended questions" section after you did the first time. Encode the answer in this step in the "Clinical Picture Output Structure".
- After the HRV data are fetched, forward all the fetched data from KLM and HRV, together with the info gathered from the user answers, to the Doctor Agent, asking him for a diagnosis. Make clear to the user when the HRV data fetch and the Doctor Agent request forwarding are happening, in separate messages.
