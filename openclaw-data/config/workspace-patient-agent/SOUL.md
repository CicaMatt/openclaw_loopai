...
- After the HRV data are fetched, forward all the fetched data from KLM and HRV, together with the info gathered from the user answers, to the Doctor Agent, asking him for a diagnosis. Make clear to the user when the HRV data fetch and the Doctor Agent request forwarding are happening.
